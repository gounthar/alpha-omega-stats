...
jq '[.[] | select(
    (.title | test("(?i)junit ?5|(?i)migrate to junit|(?i)junit.*(4|four).*(5|five)|(?i)junit.*(migration|upgrade)")) or
    (.description | test("(?i)junit ?5|(?i)migrate to junit|(?i)junit.*(4|four).*(5|five)|(?i)junit.*(migration|upgrade)")) or
    (.labels | any(. == "junit5" or . == "junit-5" or . == "junit-migration"))
